...
namespace AdvancedSceneManager.Editor
{
    ...
    public class SceneField : ObjectField, INotifyValueChanged<Scene>
    {
        ...
        void SetupDragDropTarget()
        {
            ...
            element.RegisterCallback<DragLeaveEvent>(e => Cancel(), TrickleDown.TrickleDown);

            void DragUpdated(DragUpdatedEvent e)
            {

                if (!HasSceneAsset(out var scene) || scene == value)
                    return;
            ...
            void Cancel() => objectType = typeof(Scene);
        ...
        public static IEnumerable<Scene> GetDragDropScenes() =>
            DragAndDrop.objectReferences.OfType<Scene>().Concat(
                ...
                OfType<SceneAsset>().
                Select(o => SceneImportUtility.GetImportedScene(AssetDatabase.GetAssetPath(o), out var s) ? s : null).
                NonNull());
