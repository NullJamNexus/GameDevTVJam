...
namespace AdvancedSceneManager.Editor.Utility
{
    ...
    public static class AssetDatabaseUtility
    {
        ...
        public static bool CreateFolder(string path, [NotNullWhen(true)] out string createdFolder)
        {
            ...
            if (string.IsNullOrEmpty(path?.Replace("/", "").Trim()))
                return false;

            var folder = Application.dataPath + ForwardSlash + path.Replace(Application.dataPath, "").Trim();
            folder = folder.Replace("/Assets/Assets", "/Assets/").Replace("//", "/");
