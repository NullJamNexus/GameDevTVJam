...
namespace KreliStudio
{
    public static class GitGUIStyle
    {
        ...
        public static readonly GUIContent closeLabel = LoadCustomIcon("winbtn_win_close");
        ...
        /// <summary>
        /// Loads a custom icon from the 'Assets/Editor Default Resources/Icons/' folder.
        /// </summary>
        private static GUIContent LoadCustomIcon(string iconName)
        {
            var icon = EditorGUIUtility.Load($"Icons/{iconName}.png") as Texture2D;
            if (icon == null)
            {
                // Fallback icon if custom icon not found
                icon = EditorGUIUtility.IconContent("d_console.erroricon").image as Texture2D;
            }
